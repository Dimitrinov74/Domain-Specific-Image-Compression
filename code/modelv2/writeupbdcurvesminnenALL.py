...
OUTPUT_DIR = "/dcs/large/u2157170/code/modelv2/batch_bd_results_mbt"
LAMBDAS = [10, 25, 50, 100, 250, 500, 1000, 10000]
MBT_QUALITIES = list(range(1, 9))  # 1..8
MAX_IMAGES = 1000
...
def compute_ms_ssim(x: torch.Tensor, y: torch.Tensor) -> float:
    # try 3-level MS-SSIM first; fallback to single-scale SSIM
    try:
...
def make_strictly_increasing(arr: np.ndarray, tiny=1e-9) -> np.ndarray:
    """If arr has repeated or non-increasing entries, make it strictly increasing by adding tiny jitter."""
...
def bd_metrics(R1, P1, R2, P2):
    ...
    # ensure strictly increasing P values for PCHIP (if duplicates, jitter a tiny bit)
    P1s = make_strictly_increasing(P1s, tiny=1e-9)
    P2s = make_strictly_increasing(P2s, tiny=1e-9)

    # overlapping interval in quality domain
    ...
    # interpolate log(rate) as function of quality
    logR1 = np.log(R1s)
    logR2 = np.log(R2s)

    # ensure logR arrays are strictly increasing as function of P for inversion stage; if not, jitter logR
    ...
    # BD-quality (PSNR) invert: quality = g(logR)
...
def evaluate_dataset_vs_mbt(image_folder: str = IMAGE_FOLDER,
                            ckpt_folder: str = CKPT_FOLDER,
                            output_dir: str = OUTPUT_DIR,
                            lambdas: List[int] = LAMBDAS,
                            mbt_qualities: List[int] = MBT_QUALITIES,
    ...
    # model_metrics[lam] = list of tuples (bpp, psnr, mss)
    model_metrics = {lam: [] for lam in lambdas}
    # mbt_metrics[q] = list of tuples
    mbt_metrics = {q: [] for q in mbt_qualities}

    # preload mbt2018 models once
    mbt_models = {}
    for q in mbt_qualities:
        try:
            mbt = mbt2018(quality=q, pretrained=True, metric="mse").to(device).eval()
            mbt_models[q] = mbt
            print(f"Loaded mbt2018 quality={q}")
        except Exception as e:
            print(f"[WARN] failed to load mbt2018 q={q}: {e}")
    ...
    for idx, img_path in enumerate(files, 1):
        ...
        # run mbt models for this image and store results
        for q, mbt in mbt_models.items():
            try:
                with torch.no_grad():
                    out = mbt(x)  # compressai returns dict
                    x_hat = out.get("x_hat", None)
                    if x_hat is None:
                        # try with quant_mode round
                        try:
                            out = mbt(x, quant_mode="round")
                        except Exception:
                            pass
                        x_hat = out.get("x_hat", None)
                    if x_hat is None:
                        raise RuntimeError("mbt output missing x_hat")
                    ...
                    mbt_metrics[q].append((bpp, psnr, mss))
            except Exception as e:
                print(f"[WARN] mbt q={q} failed on image {idx}/{len(files)}: {e}")

        # run custom model for each lambda
        for lam in lambdas:
            ckpt_path = os.path.join(ckpt_folder, f"alpha{lam}lambda.pt")
            if not os.path.isfile(ckpt_path):
                # skip missing checkpoint but warn once
                # (we'll warn here for each image; keep brief)
                continue
            try:
                # load model (fresh to avoid state issues)
    ...
    df_mbt = pd.DataFrame(mbt_summary).sort_values("bpp").reset_index(drop=True)

    # Save aggregated RD points
    df_model.to_csv(os.path.join(output_dir, "agg_model_rd.csv"), index=False)
    df_mbt.to_csv(os.path.join(output_dir, "agg_mbt_rd.csv"), index=False)
    ...
    mbt_bpp = df_mbt["bpp"].values
    mbt_psnr = df_mbt["psnr"].values
    mbt_mss = df_mbt["msssim"].values

    # --------- Compute BD metrics ---------
    bd_results = {}
    try:
        bd_rate_psnr, bd_psnr = bd_metrics(model_bpp, model_psnr, mbt_bpp, mbt_psnr)
    except Exception as e:
        bd_rate_psnr, bd_psnr = float('nan'), float('nan')
        print("Warning BD PSNR failed:", e)
    try:
        bd_rate_mss, bd_mss = bd_metrics(model_bpp, model_mss, mbt_bpp, mbt_mss)
    except Exception as e:
    ...
    pd.DataFrame([bd_results]).to_csv(os.path.join(output_dir, "bd_metrics_summary.csv"), index=False)
    print("Saved BD metrics to", os.path.join(output_dir, "bd_metrics_summary.csv"))

    # --------- Plot RD curves (log x-axis as requested) ---------
    # PSNR vs bpp
    plt.figure(figsize=(8,6))
    plt.plot(model_bpp, model_psnr, "o-", label="My model (PSNR)")
    plt.plot(mbt_bpp, mbt_psnr, "s--", label="mbt2018 (PSNR)")
    plt.xscale("log")
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "rd_psnr_avg_logx.png"), dpi=300)
    plt.close()

    # MS-SSIM vs bpp
    plt.figure(figsize=(8,6))
    plt.plot(model_bpp, model_mss, "o-", label="My model (MS-SSIM)")
    plt.plot(mbt_bpp, mbt_mss, "s--", label="mbt2018 (MS-SSIM)")
    plt.xscale("log")
    plt.xlabel("Bits per pixel (bpp) [log scale]")
    plt.ylabel("MS-SSIM")
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "rd_msssim_avg_logx.png"), dpi=300)
    plt.close()
...
if __name__ == "__main__":
    results = evaluate_dataset_vs_mbt(
        image_folder=IMAGE_FOLDER,
        ckpt_folder=CKPT_FOLDER,
        output_dir=OUTPUT_DIR,
        lambdas=LAMBDAS,
        mbt_qualities=MBT_QUALITIES,
