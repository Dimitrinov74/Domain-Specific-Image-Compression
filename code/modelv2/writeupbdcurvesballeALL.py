...
# compressai model
from compressai.zoo import bmshj2018_hyperprior as bmshj
...
OUTPUT_DIR = "/dcs/large/u2157170/code/modelv2/batch_bd_results_bmshj"
LAMBDAS = [10, 25, 50, 100, 250, 500, 1000, 10000]
BMSHJ_QUALITIES = list(range(1, 9))  # 1..8
MAX_IMAGES = 1000
...
def bd_metrics(R1, P1, R2, P2):
    ...
    P1s = make_strictly_increasing(P1s)
    P2s = make_strictly_increasing(P2s)
...
# def evaluate_dataset_vs_bmshj(image_folder: str = IMAGE_FOLDER,
#                             ckpt_folder: str = CKPT_FOLDER,
#                             output_dir: str = OUTPUT_DIR,
#                             lambdas: List[int] = LAMBDAS,
#                             bmshj_qualities: List[int] = BMSHJ_QUALITIES,
...
#     model_metrics = {lam: [] for lam in lambdas}
#     bmshj_metrics = {q: [] for q in bmshj_qualities}

#     bmshj_models = {}
#     for q in bmshj_qualities:
#         try:
#             model = bmshj(quality=q, pretrained=True, metric="mse").to(device).eval()
#             bmshj_models[q] = bmshj
#             print(f"Loaded bmshj2018 quality={q}")
#         except Exception as e:
#             print(f"[WARN] failed to load bmshj2018 q={q}: {e}")
...
#             continue
#         x = tensor_from_pil(pil)
#         H, W = x.shape[2], x.shape[3]
#         num_pixels = int(H * W)

#         for q, bmshj in bmshj_models.items():
#             try:
#                 with torch.no_grad():
#                     out = bmshj(x)
...
#                     bmshj_metrics[q].append((bpp, psnr, mss))
#             except Exception as e:
#                 print(f"[WARN] bmshj q={q} failed on image {idx}/{len(files)}: {e}")
...
#     bmshj_summary = []
#     for q in bmshj_qualities:
#         vals = bmshj_metrics.get(q, [])
#         if len(vals) == 0:
#             continue
#         bpps = np.array([v[0] for v in vals], dtype=float)
#         psnrs = np.array([v[1] for v in vals], dtype=float)
#         msss = np.array([v[2] for v in vals], dtype=float)
#         bmshj_summary.append({
#             "quality": q,
#             "count": len(vals),
#             "bpp": bpps.mean(),
#             "psnr": psnrs.mean(),
...
#     df_bmshj = pd.DataFrame(bmshj_summary).sort_values("bpp").reset_index(drop=True)

#     df_model.to_csv(os.path.join(output_dir, "balle_agg_model_rd.csv"), index=False)
#     df_bmshj.to_csv(os.path.join(output_dir, "balle_agg_bmshj_rd.csv"), index=False)
...
#     bmshj_bpp = df_bmshj["bpp"].values
#     bmshj_psnr = df_bmshj["psnr"].values
#     bmshj_mss = df_bmshj["msssim"].values

#     bd_results = {}
#     try:
#         bd_rate_psnr, bd_psnr = bd_metrics(model_bpp, model_psnr, bmshj_bpp, bmshj_psnr)
#     except Exception as e:
#         bd_rate_psnr, bd_psnr = float('nan'), float('nan')
#         print("Warning BD PSNR failed:", e)
#     try:
#         bd_rate_mss, bd_mss = bd_metrics(model_bpp, model_mss, bmshj_bpp, bmshj_mss)
#     except Exception as e:
...
#     pd.DataFrame([bd_results]).to_csv(os.path.join(output_dir, "balle_bd_metrics_summary.csv"), index=False)
#     print("Saved BD metrics to", os.path.join(output_dir, "balle_bd_metrics_summary.csv"))

#     plt.figure(figsize=(8,6))
#     plt.plot(model_bpp, model_psnr, "o-", label="My model (PSNR)")
#     plt.plot(bmshj_bpp, bmshj_psnr, "s--", label="bmshj2018 (PSNR)")
#     plt.xscale("log")
...
#     plt.legend()
#     plt.tight_layout()
#     plt.savefig(os.path.join(output_dir, "balle_rd_psnr_avg_logx.png"), dpi=300)
#     plt.close()

#     # MS-SSIM vs bpp
#     plt.figure(figsize=(8,6))
#     plt.plot(model_bpp, model_mss, "o-", label="My model (MS-SSIM)")
#     plt.plot(bmshj_bpp, bmshj_mss, "s--", label="bmshj2018 (MS-SSIM)")
#     plt.xscale("log")
#     plt.xlabel("Bits per pixel (bpp) [log scale]")
#     plt.ylabel("MS-SSIM")
...
#     plt.legend()
#     plt.tight_layout()
#     plt.savefig(os.path.join(output_dir, "balle_rd_msssim_avg_logx.png"), dpi=300)
#     plt.close()
...
def evaluate_dataset_vs_bmshj(image_folder: str = IMAGE_FOLDER,
    ...
    for q in bmshj_qualities:
        try:
            # instantiate the model and store the instance (not the constructor)
            inst = bmshj(quality=q, pretrained=True, metric="mse").to(device).eval()
            bmshj_models[q] = inst
            print(f"Loaded bmshj2018_hyperprior quality={q} (pretrained=True)")
        except Exception as e:
            # try fallback without pretrained if pretrained weights unavailable
            try:
                inst = bmshj(quality=q, pretrained=False, metric="mse").to(device).eval()
                bmshj_models[q] = inst
                print(f"Loaded bmshj2018_hyperprior quality={q} (pretrained=False)")
            except Exception as e2:
                print(f"[WARN] failed to load bmshj2018 q={q}: {e2}")

    if len(bmshj_models) == 0:
        print("[WARN] No bmshj models loaded; bmshj comparisons will be skipped.")
    ...
    for idx, img_path in enumerate(files, 1):
        ...
        # run bmshj models for this image and store results
        for q, bmshj_model in bmshj_models.items():
            try:
                with torch.no_grad():
                    out = bmshj_model(x)  # use instantiated model
                    x_hat = out.get("x_hat", None)
                    if x_hat is None:
                        try:
                            out = bmshj_model(x, quant_mode="round")
                        except Exception:
                            pass
                        x_hat = out.get("x_hat", None)
                    if x_hat is None:
    ...
    # handle empty baseline gracefully
    if len(bmshj_summary) == 0:
        print("[WARN] No bmshj results collected; creating empty baseline DataFrame and skipping BD/plots for baseline.")
        df_bmshj = pd.DataFrame(columns=["quality", "count", "bpp", "psnr", "msssim"])
    else:
        df_bmshj = pd.DataFrame(bmshj_summary).sort_values("bpp").reset_index(drop=True)
    ...
    model_bpp = df_model["bpp"].values if not df_model.empty else np.array([])
    model_psnr = df_model["psnr"].values if not df_model.empty else np.array([])
    model_mss = df_model["msssim"].values if not df_model.empty else np.array([])

    bmshj_bpp = df_bmshj["bpp"].values if not df_bmshj.empty else np.array([])
    bmshj_psnr = df_bmshj["psnr"].values if not df_bmshj.empty else np.array([])
    bmshj_mss = df_bmshj["msssim"].values if not df_bmshj.empty else np.array([])

    bd_results = {"bd_rate_psnr_pct": float('nan'), "bd_psnr_db": float('nan'),
                  "bd_rate_mss_pct": float('nan'), "bd_mss_diff": float('nan')}

    # Only attempt BD if both curves have >= 2 points
    if len(model_bpp) >= 2 and len(bmshj_bpp) >= 2:
        try:
            bd_rate_psnr, bd_psnr = bd_metrics(model_bpp, model_psnr, bmshj_bpp, bmshj_psnr)
            bd_results["bd_rate_psnr_pct"] = bd_rate_psnr
            bd_results["bd_psnr_db"] = bd_psnr
        ...
        print("[INFO] Not enough points to compute BD metrics (need >=2 points for both curves).")

    pd.DataFrame([bd_results]).to_csv(os.path.join(output_dir, "balle_bd_metrics_summary.csv"), index=False)
    print("Saved BD metrics to", os.path.join(output_dir, "balle_bd_metrics_summary.csv"))

    # Plotting guarded by non-empty data
    if not df_model.empty and not df_bmshj.empty:
        try:
            plt.figure(figsize=(8,6))
            plt.plot(model_bpp, model_psnr, "o-", label="My model (PSNR)")
            plt.plot(bmshj_bpp, bmshj_psnr, "s--", label="bmshj2018 (PSNR)")
            ...
            plt.xlabel("Bits per pixel (bpp) [log scale]")
            plt.ylabel("PSNR (dB)")
            plt.title(f"PSNR vs bpp (averaged over images)\nBD-Rate: {bd_results['bd_rate_psnr_pct']:.2f}%, BD-PSNR: {bd_results['bd_psnr_db']:.3f} dB")
            plt.grid(True, which="both", ls="--", alpha=0.6)
            plt.legend()
            plt.tight_layout()
            plt.savefig(os.path.join(output_dir, "balle_rd_psnr_avg_logx.png"), dpi=300)
        ...
        try:
            ...
            plt.xlabel("Bits per pixel (bpp) [log scale]")
            plt.ylabel("MS-SSIM")
            plt.title(f"MS-SSIM vs bpp (averaged over images)\nBD-Rate: {bd_results['bd_rate_mss_pct']:.2f}%, BD-MS-SSIM: {bd_results['bd_mss_diff']:.4f}")
            plt.grid(True, which="both", ls="--", alpha=0.6)
            plt.legend()
            plt.tight_layout()
            plt.savefig(os.path.join(output_dir, "balle_rd_msssim_avg_logx.png"), dpi=300)
        ...
        print("[INFO] Skipping RD plots because one of the averaged RD tables is empty.")
...
if __name__ == "__main__":
    results = evaluate_dataset_vs_bmshj(
        image_folder=IMAGE_FOLDER,
        ckpt_folder=CKPT_FOLDER,
        output_dir=OUTPUT_DIR,
        lambdas=LAMBDAS,
        bmshj_qualities=BMSHJ_QUALITIES,
